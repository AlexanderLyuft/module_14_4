...
initiate_db()
add_product('Протеиновый батончик', 'Утоляет голод после тренировки, дает энергию', 100)
add_product('ВСАА', 'Способствует восстановлению мышечной ткани', 200)
add_product('Протеиновый коктейль', 'Поддерживает иммунитет, создает и восстанавливает клетки и ткани', 300)
add_product('Гейнер', 'Помогает увеличить мышечный объем и набрать вес', 400)
...
async def process_weight(message: types.Message, state: FSMContext):
    ...
    # Здесь мы просто используем формулу для мужчин для примера.
